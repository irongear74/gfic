...
type
  TFloppyFormat = record
    ...
    RootDirSectors: Word;  // vypočítané
  end;
...
procedure CreateBlankFAT12(const FileName: string; const fmt: TFloppyFormat);
...
begin
  ...
  try
    // Boot sektor
    SetLength(BootSector, fmt.BytesPerSector);
    FillChar(BootSector[0], Length(BootSector), 0);

    BootSector[0] := $EB;  // JMP short
    BootSector[1] := $3C;
    BootSector[2] := $90;  // NOP
    Move('MSDOS5.0', BootSector[3], 8);

    PWord(@BootSector[$0B])^ := fmt.BytesPerSector;
    BootSector[$0D] := fmt.SectorsPerCluster;
    PWord(@BootSector[$0E])^ := fmt.ReservedSectors;
    BootSector[$10] := fmt.NumberOfFats;
    PWord(@BootSector[$11])^ := fmt.RootDirEntries;
    PWord(@BootSector[$13])^ := fmt.TotalSectors;
    BootSector[$15] := fmt.MediaDescriptor;
    PWord(@BootSector[$16])^ := fmt.SectorsPerFat;
    PWord(@BootSector[$18])^ := fmt.SectorsPerTrack;
    PWord(@BootSector[$1A])^ := fmt.Heads;
    PLongword(@BootSector[$1C])^ := fmt.HiddenSectors;
    // Boot signature
    BootSector[$1FE] := $55;
    BootSector[$1FF] := $AA;
    ...
    FAT[1] := $FF;
    FAT[2] := $FF;
    for i := 1 to fmt.NumberOfFats do
      fs.WriteBuffer(FAT[0], Length(FAT));

    // Root dir
    SetLength(RootDir, fmt.RootDirSectors * fmt.BytesPerSector);
    ...
    begin
      RootDir := nil;
      SetLength(RootDir, fmt.BytesPerSector);
      FillChar(RootDir[0], Length(RootDir), 0);
      for i := 1 to DataSize div fmt.BytesPerSector do
        fs.WriteBuffer(RootDir[0], Length(RootDir));
    end;
  finally
    fs.Free;
